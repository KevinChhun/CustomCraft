...
@Composable
fun UpdateProfilePicture(navController: NavHostController) {
            ...
            Row {
                ...
                Button(
                    onClick = {
                        imageUri?.let { uri ->
                            scope.launch {
                                val result = updateProfileImage(uri)
                                if (result) {
...
private suspend fun updateProfileImage(uri: Uri): Boolean {
    val userId = FirebaseAuth.getInstance().currentUser?.uid
    val db = FirebaseFirestore.getInstance()
    val storage = FirebaseStorage.getInstance()
    val userDoc = db.collection("users").document(userId.toString()).get().await()
    val currentProfileImgURL = userDoc.getString("profileImgURL")
    ...
    val newImageRef = storage.reference.child("images/${userId}/profileimage/${imageUniqueID}.png")
    return try {
        val uploadTask = newImageRef.putFile(uri).await()
        db.collection("users").document(userId.toString()).update(
            "profileImgURL", "images/${userId}/profileimage/${imageUniqueID}.png"
        )
        true
    } catch (e: Exception) {
        false
